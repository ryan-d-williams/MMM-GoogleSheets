...
Module.register("MMM-GoogleSheets", {
	...
	defaults: {
		...
		smoothScroll: false,
		writeableCheckboxes: true,
		writeableDropdowns: true,
		refreshOnEdit: true
	},
	...
	notificationReceived: async function (notification) {
		if (notification == "MODULE_DOM_UPDATED") {
			...
			if (this.config.writeableCheckboxes) {
				this.attachCheckboxHandlers();
			}
			if (this.config.writeableDropdowns) {
	...
	attachCheckboxHandlers: function () {
		let self = this;
		let checkboxes = document.querySelectorAll(".gs-interactive-checkbox");
		...
		checkboxes.forEach(checkbox => {
			...
			checkbox.onclick = function (event) {
				...
				// Calculate absolute row and column in the sheet
				let absoluteRow = self.rangeStartRow + rowIndex;
				let absoluteCol = self.rangeStartCol + colIndex;

				Log.info(`Checkbox clicked: Row ${absoluteRow}, Col ${absoluteCol}, Checked: ${checked}`);

				// Send update to Google Sheets
				self.sendSocketNotification("GOOGLE_SHEETS_UPDATE_CELL", {
					url: self.config.url,
				...
				if (self.sheetData && self.sheetData[rowIndex] && self.sheetData[rowIndex][colIndex]) {
					self.sheetData[rowIndex][colIndex].val_data.is_checked = checked;
					self.sheetData[rowIndex][colIndex].value = checked ? "TRUE" : "FALSE";
				}
			};
		});
	},
	...
	attachDropdownHandlers: function () {
		let self = this;
		let dropdowns = document.querySelectorAll(".gs-interactive-dropdown");
		...
		dropdowns.forEach(dropdown => {
			...
			dropdown.onchange = function (event) {
				...
				Log.info(`Dropdown changed: Row ${absoluteRow}, Col ${absoluteCol}, Value: ${value}`);
				...
				// Optimistically update local data
				if (self.sheetData && self.sheetData[rowIndex] && self.sheetData[rowIndex][colIndex]) {
					self.sheetData[rowIndex][colIndex].val_data.dropdown_selected = value;
					self.sheetData[rowIndex][colIndex].value = value;
				}
			};
		});
	},
	...
	socketNotificationReceived: async function (notification, payload) {
		if (
			...
			if (payload.error) {
				...
				this.rangeStartRow = payload.range_start_row;
				this.rangeStartCol = payload.range_start_col;
			...
			Log.info(this.sheetData);
		} else if (
			notification == "GOOGLE_SHEETS_UPDATE_RESULT" &&
			payload.instanceId == this.identifier
		) {
			if (payload.error) {
				Log.error("Error updating cell: " + payload.error_msg);
				this.getData();
			} else {
				Log.info("Cell updated successfully");
				if (this.config.refreshOnEdit) {
					Log.info("Refreshing data after edit");
					setTimeout(() => {
						this.getData();
					}, 500);
				}
	...
	combineSheetsData(sheetData) {
		let data = sheetData.values.map((row, i) => {
			return row.map((col, j) => {
				...
				val_data.dropdown_selected = val_data.is_dropdown && sheetData.validations[i][j].dropdown_selected
